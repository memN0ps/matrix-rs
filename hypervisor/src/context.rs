...
impl Context {
    pub fn new() -> Result<Box<Context, PhysicalAllocator>, HypervisorError> {
        
        let mut context: Box<Context, PhysicalAllocator> = unsafe { Box::try_new_zeroed_in(PhysicalAllocator)?.assume_init() };

        unsafe { RtlCaptureContext(context.as_mut() as *mut _ as _) };
