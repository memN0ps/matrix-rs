use crate::{
    ...
    utils::{
        controls::{adjust_vmx_controls, segment_limit, VmxControl},
        segmentation::Segment,
    },
    vmexit_handler::vmexit_stub,
...
use x86::{
    bits64, controlregs, debugregs, dtables, msr,
    segmentation::{self},
    task,
    vmx::{
        self,
...
impl VcpuData {
    ...
    pub fn init_vmcs_region(&mut self) -> Result<(), HypervisorError> {
        self.vmcs_region_physical_address =
            physical_address(self.vmcs_region.as_ref() as *const _ as _).as_u64();
    ...
    pub fn init_vmcs_control_values(&mut self) -> Result<(), HypervisorError> {
        const PRIMARY_CTL: u64 = (PrimaryControls::HLT_EXITING.bits() | /*PrimaryControls::USE_MSR_BITMAPS.bits() |*/ PrimaryControls::SECONDARY_CONTROLS.bits())
            as u64;
        const SECONDARY_CTL: u64 = (SecondaryControls::ENABLE_RDTSCP.bits()
            | SecondaryControls::ENABLE_XSAVES_XRSTORS.bits()
            | SecondaryControls::ENABLE_INVPCID.bits()/* | SecondaryControls::ENABLE_EPT.bits() */)
            as u64;
        ...
        const EXIT_CTL: u64 = (ExitControls::HOST_ADDRESS_SPACE_SIZE.bits()
            | ExitControls::ACK_INTERRUPT_ON_EXIT.bits()) as u64;
        ...
        unsafe {
            support::vmwrite(
                x86::vmx::vmcs::control::CR0_READ_SHADOW,
                controlregs::cr0().bits() as u64,
            )?
        };
        unsafe {
            support::vmwrite(
                x86::vmx::vmcs::control::CR4_READ_SHADOW,
    ...
    pub fn init_guest_register_state(&self) -> Result<(), HypervisorError> {
        ...
        support::vmwrite(
            guest::CS_LIMIT,
            segment_limit(segmentation::cs().bits()) as _,
        )?;
        support::vmwrite(
            guest::SS_LIMIT,
            segment_limit(segmentation::ss().bits()) as _,
        )?;
        support::vmwrite(
            guest::DS_LIMIT,
            segment_limit(segmentation::ds().bits()) as _,
        )?;
        support::vmwrite(
            guest::ES_LIMIT,
            segment_limit(segmentation::es().bits()) as _,
        )?;
        support::vmwrite(
            guest::FS_LIMIT,
            segment_limit(segmentation::fs().bits()) as _,
        )?;
        support::vmwrite(
            guest::GS_LIMIT,
            segment_limit(segmentation::gs().bits()) as _,
        )?;
        unsafe {
            support::vmwrite(
                guest::LDTR_LIMIT,
                segment_limit(dtables::ldtr().bits()) as _,
            )?
        };
        ...
        support::vmwrite(
            ...
            Segment::from_selector(segmentation::cs(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        support::vmwrite(
            guest::SS_ACCESS_RIGHTS,
            Segment::from_selector(segmentation::ss(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        support::vmwrite(
            guest::DS_ACCESS_RIGHTS,
            Segment::from_selector(segmentation::ds(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        support::vmwrite(
            guest::ES_ACCESS_RIGHTS,
            Segment::from_selector(segmentation::es(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        support::vmwrite(
            guest::FS_ACCESS_RIGHTS,
            Segment::from_selector(segmentation::fs(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        support::vmwrite(
            guest::GS_ACCESS_RIGHTS,
            Segment::from_selector(segmentation::gs(), &gdt)
                .access_rights
                .bits() as _,
        )?;
        unsafe {
            support::vmwrite(
                ...
                Segment::from_selector(dtables::ldtr(), &gdt)
                    .access_rights
                    .bits() as _,
            )?
        };
        unsafe {
            support::vmwrite(
                ...
                Segment::from_selector(task::tr(), &gdt)
                    .access_rights
                    .bits() as _,
            )?
        };
        ...
        support::vmwrite(
            guest::CS_BASE,
            Segment::from_selector(segmentation::cs(), &gdt).base,
        )?;
        support::vmwrite(
            guest::SS_BASE,
            Segment::from_selector(segmentation::ss(), &gdt).base,
        )?;
        support::vmwrite(
            guest::DS_BASE,
            Segment::from_selector(segmentation::ds(), &gdt).base,
        )?;
        support::vmwrite(
            guest::ES_BASE,
            Segment::from_selector(segmentation::es(), &gdt).base,
        )?;
        unsafe {
            support::vmwrite(
                guest::LDTR_BASE,
                Segment::from_selector(dtables::ldtr(), &gdt).base,
            )?
        };
        unsafe {
            support::vmwrite(
                guest::TR_BASE,
                Segment::from_selector(task::tr(), &gdt).base,
            )?
        };
    ...
    pub fn init_host_register_state(&mut self) -> Result<(), HypervisorError> {
        ...
        // Host RSP/RIP
        let host_rsp =
            (self.host_stack_layout.as_mut() as *const _ as u64) + STACK_CONTENTS_SIZE as u64 - 8;
        ...
        support::vmwrite(
            host::CS_SELECTOR,
            (segmentation::cs().bits() & SELECTOR_MASK) as u64,
        )?;
        support::vmwrite(
            host::SS_SELECTOR,
            (segmentation::ss().bits() & SELECTOR_MASK) as u64,
        )?;
        support::vmwrite(
            host::DS_SELECTOR,
            (segmentation::ds().bits() & SELECTOR_MASK) as u64,
        )?;
        support::vmwrite(
            host::ES_SELECTOR,
            (segmentation::es().bits() & SELECTOR_MASK) as u64,
        )?;
        support::vmwrite(
            host::FS_SELECTOR,
            (segmentation::fs().bits() & SELECTOR_MASK) as u64,
        )?;
        support::vmwrite(
            host::GS_SELECTOR,
            (segmentation::gs().bits() & SELECTOR_MASK) as u64,
        )?;
        unsafe {
            support::vmwrite(
                host::TR_SELECTOR,
                (task::tr().bits() & SELECTOR_MASK) as u64,
            )?
        };
