...
use super::{
    descriptors::SegmentDescriptor, host_rsp::HostRsp, msr_bitmap::MsrBitmap, vmcs::Vmcs,
    vmxon::Vmxon,
};
...
impl Vmx {
    ...
    fn setup_guest_registers_state(&mut self, context: _CONTEXT) {
        ...
        vmwrite(guest::CS_BASE, SegmentDescriptor::from(SegmentSelector(context.SegCs)).base);
        vmwrite(guest::SS_BASE, SegmentDescriptor::from(SegmentSelector(context.SegSs)).base);
        vmwrite(guest::DS_BASE, SegmentDescriptor::from(SegmentSelector(context.SegDs)).base);
        vmwrite(guest::ES_BASE, SegmentDescriptor::from(SegmentSelector(context.SegEs)).base);
        unsafe { vmwrite(guest::FS_BASE, msr::rdmsr(msr::IA32_FS_BASE)) };
        unsafe { vmwrite(guest::GS_BASE, msr::rdmsr(msr::IA32_GS_BASE)) };
        unsafe { vmwrite(guest::LDTR_BASE, SegmentDescriptor::from(SegmentSelector(x86::dtables::ldtr().bits())).base) };
        unsafe { vmwrite(guest::TR_BASE, SegmentDescriptor::from( SegmentSelector(x86::task::tr().bits())).base) };

        vmwrite(guest::CS_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegCs)).limit);
        vmwrite(guest::SS_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegSs)).limit);
        vmwrite(guest::DS_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegDs)).limit);
        vmwrite(guest::ES_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegEs)).limit);
        vmwrite(guest::FS_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegFs)).limit);
        vmwrite(guest::GS_LIMIT, SegmentDescriptor::from(SegmentSelector(context.SegGs)).limit);
        unsafe { vmwrite(guest::LDTR_LIMIT, SegmentDescriptor::from(SegmentSelector(dtables::ldtr().bits())).limit) };
        unsafe { vmwrite(guest::TR_LIMIT, SegmentDescriptor::from(SegmentSelector(task::tr().bits())).limit) };
    ...
    fn setup_host_registers_state(&mut self, context: _CONTEXT) {
        ...
        unsafe { vmwrite(host::TR_BASE, SegmentDescriptor::from(SegmentSelector(x86::task::tr().bits())).base) };
